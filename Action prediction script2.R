...
#Accuracy################################################################################################
#Action predition accuracy spreadsheet
remove(list = ls())
my_data_original <- read.csv(file.choose()) #loads window to choose your file. Action predition accuracy spreadsheet
...
model.metrics <- augment(model_anova_accuracy_lm) %>%
  select(-.hat, -.sigma, -.fitted, -.se.fit) # Remove details
head(model.metrics, 3)
shapiro_test(model.metrics$.resid)
model.metrics %>% levene_test(.resid ~ Group) # checking variances of residuals is homogeneous
...
# Inspect the model diagnostic metrics
model.metrics <- augment(model_anova_accuracy_lm_sq) %>%
  select(-.hat, -.sigma, -.fitted, -.se.fit) # Remove details
head(model.metrics, 3)
shapiro_test(model.metrics$.resid)
model.metrics %>% levene_test(.resid ~ Group) # checking variances of residuals in homogeneous
#Shapiro Wilkes still sig: 0.978 0.00575
...
my_data_original_autistic <- read.csv(file.choose()) #Action_predition_accuracy_autistic
my_data_original_non_autistic <- read.csv(file.choose()) #Action_predition_accuracy_non_autistic
...
# Inspect the model diagnostic metrics
model.metrics <- augment(model_anova_Timing_lm) %>%
select(-.hat, -.sigma, -.fitted, -.se.fit) # Remove details
head(model.metrics, 3)
shapiro_test(model.metrics$.resid)
model.metrics %>% levene_test(.resid ~ Group) # checking variances of residuals in homogeneous

# Shapiro wilks no sig:  p=0.2,, data looks normal
#Run ANOVA 
...
remove(list = ls())
#non-autistic data
my_data_original_non_autistic <- read.csv(file.choose()) 
...
#autistic data
my_data_original_autistic <- read.csv(file.choose()) 
...
my_data_original_questionnaires <- read.csv(file.choose()) #loads window to choose your file - Questionnaires
...
remove(list = ls())
my_data_original_control_task <- read.csv(file.choose()) #loads window to choose your file
